    ...
    <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-md">
        ...
        <form method="POST" class="mt-3 grid grid-cols-2 gap-5" action="{{ route('menus.store') }}">
            ...
            <div>
                ...
                <x-select id="is_available" name="is_available" value="{{ old('is_available') }}" :error="$errors->first('is_available')">
                    <option value="1" @if (old('is_available') == '1') selected @endif>Available</option>
                    <option value="0" @if (old('is_available') == '0') selected @endif>Not Available</option>
                </x-select>
            </div>
            ...
            <div>
                ...
                <x-select id="category_id" name="category_id" default-label="Select Category..."
                    value="{{ old('category_id') }}" :error="$errors->first('category_id')">
